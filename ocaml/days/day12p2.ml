...
let solve root =
    let rec iter n visited nodes =
        ...
        | Some _ -> printf "FOUND IT!\n"; print_graph (visited @ nodes); n
    in
    let start_nodes = mark_visited [root] in
    iter 0 [root] start_nodes;;
...
let graph_of_grid grid =
    ...
    matrix_iteri (fun pos nd -> List.iter (add_link nd) @@
        ...
        nodes;
    match Array.find_opt (fun nd -> nd.c = 'E') (matrix_flatten nodes) with
    | None -> failwith "bad input: no end node"
    | Some nd -> nd;;
